...
public class QuantifiedCondition : ICondition, IBound, IConditionContainer<ICondition>
{
    ...
    public QuantifiedCondition(Operator op, string field, ICondition condition)
    {
        ...
        if (!IsQuantifier(op))
        {
            throw new WeequeryException($"Operator '{op}' on field '{field}' is not a quantifier, so it cannot be represented by {nameof(QuantifiedCondition)}: the quantifiers are {Operator.Any}, {Operator.All} and {Operator.None}");
        }
